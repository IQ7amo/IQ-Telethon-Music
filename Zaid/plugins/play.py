...
fotoplay = "https://telegra.ph/file/e22ae49d6244878708a10.jpg"
ngantri = "https://telegra.ph/file/e22ae49d6244878708a10.jpg"
...
btnn =[
    [Button.url("🕷️ پشتگیری", url=f"t.me/{Config.SUPPORT}"), Button.url("Cʜᴀɴɴᴇʟ 🙋", url=f"t.me/{Config.CHANNEL}")],
    [Button.inline("🗑 پاککردنەوە", data="cls")]]
...
async def play(event):
    ...
    if (
        ...
        return await event.client.send_file(chat_id, Config.CMD_IMG, caption="**بۆ ئەوەی شتێك پێبکەیت پێویستە ناونیشانێك بنووسیت**\n\n **نموونە**: `/play سورة الكهف`", buttons=btnn)
        ...
        botman = await edit_or_reply(event, "📥 **دادەبەزێت**")
...
#end
@Zaid.on(events.NewMessage(pattern="^[/?!]stop"))
@is_admin
async def vc_end(event, perm):
    ...
    if chat_id in QUEUE:
        try:
            await call_py.leave_group_call(chat_id)
            clear_queue(chat_id)
            await event.reply("** بە بەسەرکەوتوویی کۆتایی هات**")
        except Exception as e:
...
async def vplay(event):
    if Config.HEROKU_MODE == "ENABLE":
        await event.reply("**__ناتوانیت ئەم فرمانە بەکاربهێنیت چونکە لە ڕێکخستنەکەدا Heroku بەکاردەهێنیت__**")
        return
    ...
    if (
        ...
        return await event.client.send_file(chat_id, Config.CMD_IMG, caption="**بۆ ئەوەی ڤیدیۆ پێبکەیت پێویستە ناونیشانێك بنووسیت**\n\n **بۆ نموونە**: `/vplay قران`", buttons=btnn)
    if replied and not replied.video and not replied.document:
        razan = await event.reply("`زانیاریەکان دەناسرێنەوە چاوەڕوان بە . . .`")
        query = event.text.split(maxsplit=1)[1]
        search = ytsearch(query)
        RESOLUSI = 720
        ...
        if search == 0:
            ...
            if hm == 0:
                ...
                caption = f"💡 **ڤیدیۆکە زیادکراوە بۆ لیستی پەخشکراو »**`#{pos}`\n\n**🏷 ناونیشان:** [{songname}]({url})\n**⏱ ماوەکەی** `{duration}`\n🎧 **لەسەر داواکاری** {from_user}"
                await razan.delete()
                await event.client.send_file(chat_id, thumb, caption=caption, buttons=btnn)
            else:
                try:
        ...
        razan = await event.reply("`گەڕان بەردەوامە...`")
        query = event.text.split(maxsplit=1)[1]
        search = ytsearch(query)
        RESOLUSI = 720
        hmmm = HighQualityVideo()
        if search == 0:
            await razan.edit("**ناونیشان نەناسراوتەوە**")
        else:
            songname = search[0]
            title = search[0]
            url = search[1]
